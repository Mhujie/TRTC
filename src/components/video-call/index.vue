<template>
  <div class="video-call-section">
    <div class="video-call-section-header">你的ID是： {{loginUserInfo && (loginUserInfo.name || loginUserInfo.userId)}}</div>
    ...
    <div :class="{ 'video-conference': true, 'is-show': isShowVideoCall }">
      ...
      <div class="video-conference-list">
        <div v-for="userId in meetingUserIdList" :key="`video-${userId}`" :id="`video-${userId}`" :class="{'user-video-container': true, 'is-me': userId === loginUserInfo.userId}">
...
export default {
  ...
  methods: {
    ...
    startMeeting: function () {
      ...
      const otherParticipants = this.meetingUserIdList.filter(userId => userId !== this.loginUserInfo.userId)
    ...
    toggleVideo: function () {
      ...
      if (this.isVideoOn) {
        ...
        const muteUserList = this.muteVideoUserIdList.filter(userId => userId !== this.loginUserInfo.userId)
        this.$store.commit('updateMuteVideoUserIdList', muteUserList)
      } else {
        this.$trtcCalling.closeCamera()
        const muteUserList = this.muteVideoUserIdList.concat(this.loginUserInfo.userId)
        this.$store.commit('updateMuteVideoUserIdList', muteUserList)
      }
    },
    toggleAudio: function () {
      ...
      if (this.isAudioOn) {
        const muteUserList = this.muteAudioUserIdList.filter(userId => userId !== this.loginUserInfo.userId)
        this.$store.commit('updateMuteAudioUserIdList', muteUserList)
      } else {
        const muteUserList = this.muteAudioUserIdList.concat(this.loginUserInfo.userId)
        this.$store.commit('updateMuteAudioUserIdList', muteUserList)
      }
    },
